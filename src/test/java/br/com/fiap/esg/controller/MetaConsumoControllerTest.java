...
class MetaConsumoControllerTest {
    ...
    @Test
    @DisplayName("Deveria devolver codigo http 400 quando tipo for invalido")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deveria devolver codigo http 400 quando periodo for invalido")
    @WithMockUser
    void cadastrarCenario6() throws Exception {
        ...
        when(service.cadastrar(any())).thenThrow(new ValidacaoException("A data de fim nao pode ser anterior a data de inicio."));
    ...
    @Test
    @DisplayName("Deveria devolver codigo http 200 ao listar metas")
    @WithMockUser
    void listarCenario1() throws Exception {
        ...
        MockHttpServletResponse response = mvc.perform(get("/api/metas"))
                .andReturn().getResponse();

        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
        assertThat(response.getContentAsString()).contains("MTH");
    }

    @Test
    @DisplayName("Deveria devolver codigo http 200 ao atualizar meta")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deveria devolver codigo http 204 ao deletar meta")
    @WithMockUser
    void deletarCenario1() throws Exception {
        doNothing().when(service).deletar(1L);

        MockHttpServletResponse response = mvc.perform(delete("/api/metas/1").with(csrf()))
                .andReturn().getResponse();
    ...
    @Test
    @DisplayName("Deveria devolver codigo http 404 ao deletar meta inexistente")
    @WithMockUser
